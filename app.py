...
dialog_service_url = "http://cccxc589.pok.ibm.com:5001/generate"
...
@app.route('/api/flan_conversation', methods=['POST', 'GET'])
def getFlanConvResponse():

    convText = request.form.get('convText')
    prompt = request.form.get('prompt')
    convContext = request.form.get('context', "")

    query_suffix = convContext + "\n" + "User: " + convText  + "\n" + "Agent:"
    query = prompt + "\n" + query_suffix
    ...
    parameters_json['top_p'] = 1
    parameters_json['temperature'] = 1.3
    parameters_json['max_new_tokens'] = 64
    parameters_json['num_return_sequences'] = 1
    payload['parameters'] = parameters_json

    jsonPayload = json.dumps(payload)

    response = requests.post(dialog_service_url, headers=headers, data=jsonPayload)

    responseJson = response.json()

    if len(responseJson['response']) < 1:
        responseText = "Flan Service looks down, please check!"
    else:
        responseText = responseJson['response'][0]

    context = query_suffix + " " + responseText
...
port = os.environ.get("PORT") or os.environ.get("VCAP_APP_PORT") or 12000
